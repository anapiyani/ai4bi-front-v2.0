...
>(
  (
    ...
    const [inputValue, setInputValue] = React.useState<string>(getDateByType(date, picker));
    ...
    const calculateNewValue = (key: string) => {
      if (picker === "12hours") {
        if (flag && inputValue.slice(1, 2) === "1" && prevIntKey === "0")
          return "0" + key;
      }

      return !flag ? "0" + key : inputValue.slice(1, 2) + key;
    };
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (e.key === "Backspace" || e.key === "Delete") {
        e.preventDefault();
        const newValue = inputValue.slice(0, -1).padStart(2, "0");
        setInputValue(newValue);
        const tempDate = date ? new Date(date) : new Date();
        onDateChange?.(setDateByType(tempDate, newValue, picker, period));
        return;
...
type DateTimePickerProps = {
  ...
  hourCycle?: 12 | 24;
  placeholder?: string;
  rangeMode?: "single" | "range" | "multiple";
  cancelChoosing?: boolean;
  cancelText?: string;
  /**
